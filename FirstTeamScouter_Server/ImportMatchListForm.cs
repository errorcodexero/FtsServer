...
{
    public partial class ImportMatchListForm : Form
    {
        private long compID;
        private string importPath;
        private MySQL_ErrorCodes mySQLErrors;

        public ImportMatchListForm(long competition_id)
        {
            mySQLErrors = new MySQL_ErrorCodes();
            this.compID = competition_id;
            this.importPath = Properties.Settings.Default.MatchListImportPath;
        ...
        public void loadFileList()
        {
            if (Directory.Exists(this.importPath))
            {
                ...
                items.Clear();
                IEnumerable<string> files = Directory.EnumerateFiles(this.importPath, "*.csv", SearchOption.TopDirectoryOnly);
        ...
        private void importItems(List<string> stuffToImport)
        {
            for (int i = 0; i < stuffToImport.Count; i++)
            {
                ...
                DataSet ds = getCsvDataSet(filePath);

                sqlInsertRecord(ds, fileName, filePath);
            }
        }

        private void sqlInsertRecord(DataSet ds, string fileName, string filePath)
        {
        ...
        private static DataSet getCsvDataSet(string filePath)
        {
            string CsvConnectionString = "Provider=Microsoft Access Text Driver (*.txt, *.csv);";
            CsvConnectionString += " Data Source = " + Path.GetDirectoryName(filePath) + ";";
            CsvConnectionString += " Extended Properties = \"Text;HDR=YES;FMT=Delimited\"";

            OleDbConnection conn = new OleDbConnection(CsvConnectionString);
            ...
            string CsvSelectString = "SELECT * FROM " + Path.GetFileName(filePath);
            OleDbDataAdapter adapter = new OleDbDataAdapter(CsvSelectString, conn);
